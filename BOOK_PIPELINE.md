...
- 删除：口播废话、寒暄、重复、"大家好/谢谢大家/下节课再见"、ASR 转写噪音/乱码
- 改写：口语 → 正式书面语，长句拆分或重组，段落逻辑分明
...
脚本自动执行：
1. 合并 `clean_chapters/0*.md` → `merged_chapter.md`（添加主标题 `# 章节标题`）
